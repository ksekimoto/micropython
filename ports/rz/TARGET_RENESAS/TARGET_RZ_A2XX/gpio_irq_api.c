...
static void handle_interrupt_in(int irq_num) {
    ...
    irqs = INTC.IRQRR.WORD;
    if (irqs & (1 << irq_num)) {
        obj = channel_obj[irq_num];
        if (obj != NULL) {
            edge_req = ((INTC.ICR1.WORD >> (obj->ch * 2)) & 3);
            if (edge_req == 1) {
                irq_event = IRQ_FALL;
            } else if (edge_req == 2) {
                irq_event = IRQ_RISE;
                ...
                __I  uint8_t *reg_in = (volatile uint8_t  *)PIDR(PINGROUP(obj->pin));
...
int gpio_irq_init(gpio_irq_t *obj, PinName pin, gpio_irq_handler handler, uint32_t id) {
    ...
    // INTC settings
    InterruptHandlerRegister((IRQn_Type)(IRQ0_IRQn + obj->ch), (void (*)(uint32_t))irq_tbl[obj->ch]);
    INTC.ICR1.WORD &= ~(0x3 << shift);
    GIC_SetPriority((IRQn_Type)(IRQ0_IRQn + obj->ch), 5);
    GIC_SetConfiguration((IRQn_Type)(IRQ0_IRQn + obj->ch), 1);
    obj->int_enable = 1;
    __enable_irq();
...
void gpio_irq_set(gpio_irq_t *obj, gpio_irq_event event, uint32_t enable) {
    ...
    if ((work_icr_val & (3 << shift)) == 0) {
        ...
        GIC_DisableIRQ((IRQn_Type)(IRQ0_IRQn + obj->ch));
        /* Clear Interrupt flags */
        INTC.IRQRR.WORD &= ~(1 << obj->ch);
        INTC.ICR1.WORD  = work_icr_val;
    } else if (obj->int_enable == 1) {
        INTC.ICR1.WORD  = work_icr_val;
        GIC_EnableIRQ((IRQn_Type)(IRQ0_IRQn + obj->ch));
    } else {
        INTC.ICR1.WORD  = work_icr_val;
    }
}
...
void gpio_irq_enable(gpio_irq_t *obj) {
    int shift = obj->ch*2;
    uint16_t work_icr_val = INTC.ICR1.WORD;
