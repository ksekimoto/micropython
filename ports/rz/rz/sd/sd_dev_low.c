...
void R_OS_TaskSleep(uint32_t ms) {
    wait_us(ms * 1000);
}

uint32_t rz_intc_enable(uint32_t port) {
    return 0;
}

uint32_t rz_intc_disable(uint32_t port) {
    return 0;
}
#endif
