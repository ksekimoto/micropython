...
void rz_sd_enable(uint32_t pin, uint8_t af_no) {
    uint32_t port = GPIO_PORT(pin);
    uint8_t mask1 = GPIO_MASK1(pin);
    rz_gpio_mode_af(pin, af_no);
    PPMR(port) |= mask1;        /* Peripheral */
    uint8_t dummy;
    CPG.STBCR10.BIT.MSTP100 = 0;
    CPG.STBCR10.BIT.MSTP101 = 0;
    dummy = CPG.STBCR10.BYTE;
    (void)dummy;
    wait_us(100000);
}

void rz_sd_disable(uint32_t pin, uint8_t af_no) {
    uint32_t port = GPIO_PORT(pin);
    uint8_t mask1 = GPIO_MASK1(pin);
    rz_gpio_mode_af(pin, af_no);
    PPMR(port) |= mask1;        /* Peripheral */
    uint8_t dummy;
    CPG.STBCR10.BIT.MSTP100 = 1;
    CPG.STBCR10.BIT.MSTP101 = 1;
    dummy = CPG.STBCR10.BYTE;
    (void)dummy;
}
