...
static const volatile struct st_rspi *RSPI[] = {
    &RSPI0, &RSPI1, &RSPI2
};
...
void spi_init(spi_t *obj, PinName mosi, PinName miso, PinName sclk, PinName ssel) {
    ...
    obj->spi.spi->SPDCR.BYTE  = 0x20;  // byte access
    obj->spi.spi->SPCKD.BYTE  = 0x00;  // SSL -> enable CLK delay : 1RSPCK
    obj->spi.spi->SSLND.BYTE  = 0x00;  // CLK end -> SSL neg delay : 1RSPCK
    obj->spi.spi->SPND.BYTE   = 0x00;  // delay between CMD : 1RSPCK + 2P1CLK
...
static inline void spi_write(spi_t *obj, int value) {
    if (obj->spi.bits == 8) {
        ...
        obj->spi.spi->SPDR.WORD.L = (uint16_t)value;
    } else {
        obj->spi.spi->SPDR.LONG    = (uint32_t)value;
    }
}
...
static inline int spi_read(spi_t *obj) {
    ...
    if (obj->spi.bits == 8) {
        ...
        read_data = obj->spi.spi->SPDR.WORD.L;
    } else {
...
static const IRQn_Type irq_set_tbl[SPI_NUM][IRQ_NUM] = {
    {SPRI0_IRQn, SPEI0_IRQn},
    {SPRI1_IRQn, SPEI1_IRQn},
    {SPRI2_IRQn, SPEI2_IRQn},
};
...
static void spi_rx_irq(uint32_t index)
{
...
static void spi_err_irq(uint32_t index)
{
    spi_t *obj = spi_data[index].async_obj;
