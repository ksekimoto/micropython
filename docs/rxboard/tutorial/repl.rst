...
interactive MicroPython prompt that you can access on the rxboard.  Using the
...
a normal serial connection. Initial versions of MicroPython for rxboard
...
To use WebREPL connect your computer to the rxboard's access point
(MicroPython-xxxxxx, see the previous section about this).  If you have
already reconfigured your rxboard to connect to a router then you can
skip this part.

Once you are on the same network as the rxboard you click the "Connect" button
(if you are connecting via a router then you may need to change the IP address,
by default the IP address is correct when connected to the rxboard's access
...
(up to 8 on the rxboard).  To recall previous lines use the up and down arrow
keys.
