...
#define RIIC_COUNT  (4)
static const volatile struct st_riic *RIIC[] = {
...
static inline int i2c_status(i2c_t *obj) {
    return obj->i2c.i2c->ICSR2.BYTE.LL;
}

static void i2c_reg_reset(i2c_t *obj) {
    /* full reset */
    obj->i2c.i2c->ICCR1.BYTE.LL &= ~CR1_ICE; // CR1.ICE off
    obj->i2c.i2c->ICCR1.BYTE.LL |=  CR1_RST; // CR1.IICRST on
    obj->i2c.i2c->ICCR1.BYTE.LL |=  CR1_ICE; // CR1.ICE on

    obj->i2c.i2c->ICMR1.BYTE.LL  =  0x08;    // P_phi /x  9bit (including Ack)
    obj->i2c.i2c->ICSER.BYTE.LL  =  0x00;    // no slave addr enabled

    /* set frequency */
    obj->i2c.i2c->ICMR1.BYTE.LL |=  obj->i2c.pclk_bit;
    obj->i2c.i2c->ICBRL.BYTE.LL  =  obj->i2c.width_low;
    obj->i2c.i2c->ICBRH.BYTE.LL  =  obj->i2c.width_hi;

    obj->i2c.i2c->ICMR2.BYTE.LL  =  0x07;
    obj->i2c.i2c->ICMR3.BYTE.LL  =  0x00;

    obj->i2c.i2c->ICFER.BYTE.LL  =  0x72;    // SCLE, NFE enabled, TMOT
    obj->i2c.i2c->ICIER.BYTE.LL  =  0x00;    // no interrupt

    obj->i2c.i2c->ICCR1.LONG &= ~CR1_RST;   // CR1.IICRST negate reset
}
...
static int i2c_set_STOP(i2c_t *obj) {
    /* SR2.STOP = 0 */
    obj->i2c.i2c->ICSR2.LONG &= ~SR2_STOP;
    /* Stop condition */
    obj->i2c.i2c->ICCR2.LONG |= CR2_SP;
...
static void i2c_set_SR2_NACKF_STOP(i2c_t *obj) {
    /* SR2.NACKF = 0 */
    obj->i2c.i2c->ICSR2.LONG &= ~SR2_NACKF;
    /* SR2.STOP = 0 */
    obj->i2c.i2c->ICSR2.LONG &= ~SR2_STOP;
}
...
static void i2c_set_MR3_NACK(i2c_t *obj) {
    /* send a NOT ACK */
    obj->i2c.i2c->ICMR3.LONG |=  MR3_ACKWP;
    obj->i2c.i2c->ICMR3.LONG |=  MR3_ACKBT;
    obj->i2c.i2c->ICMR3.LONG &= ~MR3_ACKWP;
}
...
static inline void i2c_power_enable(i2c_t *obj) {
    ...
    switch ((int)obj->i2c.index) {
        case I2C_0:
            CPG.STBCR8.BIT.MSTP87 = 0;
            break;
        case I2C_1:
            CPG.STBCR8.BIT.MSTP86 = 0;
            break;
        case I2C_2:
            CPG.STBCR8.BIT.MSTP85 = 0;
            break;
        case I2C_3:
            CPG.STBCR8.BIT.MSTP84 = 0;
            break;
    }
    dummy = CPG.STBCR8.BYTE;
    (void)dummy;
}
...
inline int i2c_start(i2c_t *obj) {
    ...
    while ((obj->i2c.i2c->ICCR2.LONG & CR2_BBSY) != 0) {
        timeout ++;
    ...
    /* Start Condition */
    obj->i2c.i2c->ICCR2.BYTE.LL |= CR2_ST;
...
static inline int i2c_do_read(i2c_t *obj, int last) {
    if (last == 2) {
        ...
        obj->i2c.i2c->ICMR3.LONG |= MR3_WAIT;
    } else if (last == 1) {
    ...
    /* return the data */
    return (obj->i2c.i2c->ICDRR.LONG & 0xFF);
}
...
void i2c_frequency(i2c_t *obj, int hz) {
    ...
    if (false == RZ_A2_IsClockMode0()) {
        pclk_val = (float64_t)CM1_RENESAS_RZ_A2_P0_CLK;
    } else {
        pclk_val = (float64_t)CM0_RENESAS_RZ_A2_P0_CLK;
    }
...
int i2c_read(i2c_t *obj, int address, char *data, int length, int stop) {
    ...
    /* check ACK/NACK */
    if ((status != 0) || ((obj->i2c.i2c->ICSR2.LONG & SR2_NACKF) != 0)) {
        /* Slave sends NACK */
        (void)i2c_set_STOP(obj);
        /* dummy read */
        value = obj->i2c.i2c->ICDRR.LONG;
        (void)i2c_wait_STOP(obj);
        i2c_set_SR2_NACKF_STOP(obj);
        obj->i2c.last_stop_flag = 1;
        return I2C_ERROR_NO_SLAVE;
    ...
    if (length != 0) {
        ...
        if (length > 2) {
            ...
            i2c_set_MR3_NACK(obj);
            data[count] = (char)obj->i2c.i2c->ICDRR.LONG;
            count++;
        } else {
            /* length == 1 */
            /* Set MR3 WAIT bit is 1 */;
        ...
        if (stop) {
            (void)i2c_set_STOP(obj);
            /* RIICnDRR read */
            value = (obj->i2c.i2c->ICDRR.LONG & 0xFF);
            data[count] = (char)value;
            /* RIICnMR3.WAIT = 0 */
            obj->i2c.i2c->ICMR3.LONG &= ~MR3_WAIT;
            (void)i2c_wait_STOP(obj);
...
int i2c_byte_write(i2c_t *obj, int data) {
    ...
    if (status != 0) {
        ...
        /* check ACK/NACK */
        if ((obj->i2c.i2c->ICSR2.LONG & SR2_NACKF) != 0) {
            /* NACK */
            i2c_set_SR2_NACKF_STOP(obj);
        } else {
            ack = 1;
...
void i2c_slave_mode(i2c_t *obj, int enable_slave) {
    if (enable_slave != 0) {
        obj->i2c.i2c->ICSER.LONG |= SER_SAR0E;   // only slave addr 0 is enabled
    } else {
        obj->i2c.i2c->ICSER.LONG &= ~SER_SAR0E;  // no slave addr enabled
    }
}
...
int i2c_slave_read(i2c_t *obj, char *data, int length) {
    ...
    for (count = 0; ((count < (length + 1)) && (break_flg == 0)); count++) {
        ...
        if (break_flg == 0) {
            if (count == 0) {
                /* dummy read */
                (void)obj->i2c.i2c->ICDRR.LONG;
            } else {
                data[count - 1] = (char)(obj->i2c.i2c->ICDRR.LONG & 0xFF);
            }
        }
    }
    if (break_flg == 0) {
        ...
        if ((i2c_status(obj) & SR2_RDRF) != 0) {
            if (count <= 1) {
                ...
                data[count - 2] = (char)(obj->i2c.i2c->ICDRR.LONG & 0xFF);
            }
        }
    }
    /* SR2.STOP = 0 */
...
int i2c_slave_write(i2c_t *obj, const char *data, int length) {
    ...
    while ((count < length) && (status == 0)) {
        ...
        if(status == 0) {
            /* Wait send end */
            status = i2c_wait_TEND(obj);
            if ((status != 0) || ((count < (length - 1)) && ((obj->i2c.i2c->ICSR2.LONG & SR2_NACKF) != 0))) {
                /* NACK */
                break;
            }
        }
...
void i2c_slave_address(i2c_t *obj, int idx, uint32_t address, uint32_t mask) {
    obj->i2c.i2c->ICSAR0.LONG = (address & 0xfffffffe);
}
...
static const IRQn_Type irq_set_tbl[RIIC_COUNT][IRQ_NUM] = {
    {INTRIICTEI0_IRQn, INTRIICRI0_IRQn, INTRIICALI0_IRQn, INTRIICTMOI0_IRQn},
    {INTRIICTEI1_IRQn, INTRIICRI1_IRQn, INTRIICALI1_IRQn, INTRIICTMOI1_IRQn},
    {INTRIICTEI2_IRQn, INTRIICRI2_IRQn, INTRIICALI2_IRQn, INTRIICTMOI2_IRQn},
    {INTRIICTEI3_IRQn, INTRIICRI3_IRQn, INTRIICALI3_IRQn, INTRIICTMOI3_IRQn},
};
...
static void i2c_tx_irq(uint32_t index)
{
    ...
    if (obj->tx_buff.pos == obj->tx_buff.length) {
        ...
        /* Clear TEND */
        obj->i2c.i2c->ICSR2.LONG &= ~(SR2_TEND);
...
static void i2c_rx_irq(uint32_t index)
{
    i2c_t *obj = i2c_data[index].async_obj;
        ...
        switch (obj->rx_buff.length - obj->rx_buff.pos) {
            case 1:
                ...
                if (i2c_data[index].shouldStop) {
                    (void)i2c_set_STOP(obj);
                    /* RIICnDRR read */
                    *(uint8_t *)obj->rx_buff.buffer = obj->i2c.i2c->ICDRR.LONG & 0xFF;
                    /* RIICnMR3.WAIT = 0 */
                    obj->i2c.i2c->ICMR3.LONG &= ~MR3_WAIT;
                    (void)i2c_wait_STOP(obj);
                    i2c_set_SR2_NACKF_STOP(obj);
...
static void i2c_err_irq(uint32_t index)
{
    i2c_t *obj = i2c_data[index].async_obj;
    i2c_abort_asynch(obj);
...
{
    int i;
    const IRQn_Type *irqTable = irq_set_tbl[obj->i2c.index];
    const IRQHandler *handlerTable = hander_set_tbl[obj->i2c.index];
    for (i = 0; i < IRQ_NUM; ++i) {
        if (enable) {
    ...
    obj->i2c.i2c->ICIER.BYTE.LL = enable ? 0x63 : 0x00;
}
...
void i2c_transfer_asynch(i2c_t *obj, const void *tx, size_t tx_length, void *rx, size_t rx_length, uint32_t address, uint32_t stop, uint32_t handler, uint32_t event, DMAUsage hint)
{
    ...
    MBED_ASSERT((obj->i2c.i2c->ICSER.LONG & SER_SAR0E) == 0); /* Slave mode */
    ...
    obj->rx_buff.width = 8;
    i2c_data[obj->i2c.index].async_obj = obj;
    i2c_data[obj->i2c.index].async_callback = handler;
    i2c_data[obj->i2c.index].event = 0;
    i2c_data[obj->i2c.index].shouldStop = stop;
    i2c_data[obj->i2c.index].address = address;
    i2c_irqs_set(obj, 1);
    ...
    if (obj->i2c.last_stop_flag != 0) {
        if (i2c_start(obj) != 0) {
            i2c_set_err_noslave(obj);
            i2c_data[obj->i2c.index].event = I2C_EVENT_ERROR | I2C_EVENT_ERROR_NO_SLAVE;
            i2c_abort_asynch(obj);
            ((void (*)())handler)();
            return;
        }
...
{
    return i2c_data[obj->i2c.index].event;
}

uint8_t i2c_active(i2c_t *obj)
{
    return i2c_data[obj->i2c.index].async_obj != NULL;
}

void i2c_abort_asynch(i2c_t *obj)
{
    i2c_data[obj->i2c.index].async_obj = NULL;
    i2c_irqs_set(obj, 0);
    i2c_reg_reset(obj);
}
